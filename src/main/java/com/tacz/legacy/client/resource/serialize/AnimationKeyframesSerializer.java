...
public class AnimationKeyframesSerializer implements JsonDeserializer<AnimationKeyframes> {
    ...
    public AnimationKeyframes deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
        ...
        if (json.isJsonPrimitive()) {
            if (json.getAsJsonPrimitive().isString()) {
                TACZLegacy.INSTANCE.getLogger().debug("Molang is not supported: \"{}\"", json.getAsString());
        ...
        if (json.isJsonObject()) {
            ...
            for (Map.Entry<String, JsonElement> entrySet : jsonObject.entrySet()) {
                ...
                try {
                    ...
                    TACZLegacy.INSTANCE.getLogger().debug("Molang is not supported: \"{}\"", entrySet.getKey());
                    return new AnimationKeyframes(keyframes);
                }
    ...
    private Vector3f readVector3f(JsonArray array) {
        ...
        float x = readVector3fElement(xElement);
        float y = readVector3fElement(yElement);
        float z = readVector3fElement(zElement);
        return new Vector3f(x, y, z);
    }

    private float readVector3fElement(JsonElement element) {
        if (element.getAsJsonPrimitive().isString()) {
            TACZLegacy.INSTANCE.getLogger().debug("Molang is not supported: \"{}\"", element.getAsString());
            return 0;
        } else {
