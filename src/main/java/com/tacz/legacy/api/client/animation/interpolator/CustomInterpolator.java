...
import com.tacz.legacy.api.client.animation.AnimationChannelContent.LerpMode;
...
public class CustomInterpolator implements Interpolator {
    ...
    private float[] getValue(int index, boolean needOffset) {
        ...
        int offset = 0;
        if (needOffset) {
            int len = content.values[index].length;
            if (len == 8) {
                offset = 4;
            } else if (len == 6) {
                offset = 3;
    ...
    private float[] doCatmullromLerp(int indexFrom, int indexTo, float alpha) {
        ...
        vx[0] = valuePrev[0]; vy[0] = valuePrev[1]; vz[0] = valuePrev[2];
        vx[1] = valueFrom[0]; vy[1] = valueFrom[1]; vz[1] = valueFrom[2];
        vx[2] = valueTo[0];   vy[2] = valueTo[1];   vz[2] = valueTo[2];
        vx[3] = valueNext[0]; vy[3] = valueNext[1]; vz[3] = valueNext[2];
        return new float[]{
