...
- 若需求更像动画状态机、关键帧插值、bone animation application、第一人称 hand/scope 渲染链路、GunSoundPlayManager 真实播放、muzzle flash / shell ejection，请优先标记为**2 号渲染 Agent 的剩余子轨**
- 若需求更像 GUI/掉落物/创造栏/展示框里的 gun-ammo-attachment 纯贴图表现、slot texture、block display 的 GUI/world 渲染与 transform 消费，请优先标记为**1 号渲染 Agent 的剩余子轨**
- 若需求更像子弹速度、下坠、tracer、部分枪打不出子弹、inventory ammo 消耗 TODO，请优先标记为**Combat/Network 的剩余 parity 子轨**，不要把弹道问题误判成纯渲染缺口
